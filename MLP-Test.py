...
MODEL_NAME = 'mlp'  # não contém 'vit' nem 'convnext' → usa ReduceLROnPlateau
...
if args.scores:
    ...
    for split in [df_train, df_val, df_test]:   # aplicar nos 3 splits
...
# ── Features de data (imputação baseada no treino) ────────────────────────────
if args.datefeatures:
...
# ── Escalonamento (sempre recomendado para MLP) ───────────────────────────────
print("Escalonando dados para a MLP...")
scaler = MinMaxScaler()
# MinMaxScaler aceita matrizes esparsas, mas converte para denso no fit_transform
X_train_final = scaler.fit_transform(X_train_final)
X_val_final   = scaler.transform(X_val_final)
X_test_final  = scaler.transform(X_test_final)
joblib.dump(scaler, f"./joblib/scaler-{args.exp}.joblib")

# Garantir arrays densos (caso ainda sejam sparse após o scaler)
...
# ── Conversão para Tensores e DataLoaders ─────────────────────────────────────
...
print(f"\nMLP criada com input_dim={input_dim}, hidden_dim={args.hidden_dim}, "
      f"dropout={args.dropout}, device={DEVICE}\n")
...
# ── Carrega o melhor checkpoint antes do teste ────────────────────────────────
...
print(f"\nLog salvo em ./logs/{args.exp}-relatory.txt")